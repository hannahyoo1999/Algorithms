...
function entries(obj) {
    ...
    for (const eachKey in obj) {
        if (obj.hasOwnProperty(eachKey)) { //check if proto is there
            arr.push([eachKey, obj[eachKey]]) //array within array
        }
    }
    return arr
}
...
// hasOwnProperty("") searches if a key exists
// for(const eachKey in obj) will loop through an object
// obj[eachKey] = value 
...
function insert(tableName, columnValuePairs) {
    ...
    for (const eachKey in columnValuePairs) {
        if (columnValuePairs.hasOwnProperty(eachKey)) {
            let val = columnValuePairs[eachKey]
