...
// 
function rotateStr(str, amnt) {
    // var newStr = str.slice(str.length - amnt, str.length) + str.slice(0, str.length - amnt);
    // return newStr
    var split = amnt % str.length; // we would do this in case we need to rotate more than str.length then instead of rotating a neg number we rotate modulus
    newStr = str.substring(str.length - split, str.length) + str.substring(0, str.length - split)
    return newStr
}
...
function isRotation(s1, s2) {
    // let match = false;
    // for (let i = 0; (i < s1.length - 1) & !match; i++) {
    //     match = s2.substring(i, s2.length) + s2.substring(0, i) === s1;
    // }
    // return match
