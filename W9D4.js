...
function santasNaughtyList(persons, badHabit) {
    ...
    for(person of persons){
        for(habit of person.habits){ // can be wrote (habit of person["habits"])
            if(habit === badHabit && badHabit !== undefined){
                badPeople.push(`${person.firstName} ${person.lastName}`) //badPeople.push(person['firstName']+' '+person['lastName'])
            }
        }
    }
    return badPeople
...
//             badPeople.push(obj.firstName, obj.lastName)
//     });
//     return badPeople
// }
...
function santasNaughtyListFunctional(persons, badHabit) {
    ...
    persons.forEach(person=>{
        if(person.habits.includes(badHabit)){
            naughtList.push(person.firstName+ " "+person.lastName)
        }
    })
    return naughtList
}
...
const santasNaughtyListFunctional = (persons, badHabit) => {
    persons
        ...
        .map(eachPerson => `${eachPerson.firstName} ${eachPerson.lastName}`)
}
