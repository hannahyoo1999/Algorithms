...
//     for(let i=0; i<nums.length; i++){ //first index
//         for(let j=0; j<nums.length; j++){ //second index
//             if(nums[i] + nums[j] == targetSum && nums[j] != nums[i]){
//                 return [i,j]
//             }
//         }
//     }
...
function twoSum(nums, targetSum){
    ...
    for(let i=0; i<nums.length; i++){
        if(hashMap.hasOwnProperty(nums[i])){ //check if the key exists in the hashMap
