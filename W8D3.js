...
function measureWaterLevels(waterLevels) {
    let rise = -1 //base case 
    let lowest = waterLevels[0] 
    let highest = waterLevels[0]
    for (let i = 1; i < waterLevels.length; i++){
        if(waterLevels[i]>lowest){ //track the rise
            highest = waterLevels[i]
            rise = highest - lowest
        }
        if (waterLevels[i] <= lowest){ //reset the pointer
            lowest = waterLevels[i]
            highest = waterLevels[i]
        }
    }
    return rise
