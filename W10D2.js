...
function groupObjects(items) {
    ...
    for (object of items) {
        let lowerCaseCategory = object.category.toLowerCase()

        if (!hashTable.hasOwnProperty(hashTable[lowerCaseCategory])) {
            hashTable[lowerCaseCategory] = []
        }

        hashTable[lowerCaseCategory].push(object)
        console.log(object)
    }
    // return hashTable
}
